...
def random_rotate3D(img_numpy, min_angle, max_angle):
    ...
    # cropping
    interval_x_min = int((np.shape(rotated_img)[0] - 128) / 2)
    interval_y_min = int((np.shape(rotated_img)[1] - 16) / 2)
    interval_z_min = int((np.shape(rotated_img)[2] - 128) / 2)

    interval_x_max = np.shape(rotated_img)[0] - 128 - interval_x_min
    interval_y_max = np.shape(rotated_img)[1] - 16 - interval_y_min
    interval_z_max = np.shape(rotated_img)[2] - 128 - interval_z_min
