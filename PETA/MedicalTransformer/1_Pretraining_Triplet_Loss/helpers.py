...
def sample_loader(phase, images, labels, indexes, args, shuffle=True, drop_last=False):
    ...
    m = nn.AvgPool3d(2, stride=2) # reduce a [96,96,114]
    if args.is_pool == 1:
        images_tensor = m(torch.from_numpy(images))
    else:
    ...
    if phase == 'train':
        ...
        #augment_compose = augment3D.RandomChoice(transforms=[augment3D.RandomShift(max_percentage=0.1),
        #                                                     augment3D.RandomRotation(min_angle=-5, max_angle=5),
        #                                                     augment3D.RandomZoom(min_percentage=0.8, max_percentage=1.2)], p=0.5)
        # augment_compose = augment3D.RandomChoice(transforms=[augment3D.RandomShift(max_percentage=0.1)], p=0.5)
        # augment_compose = augment3D.RandomChoice(transforms=[augment3D.RandomZoom(min_percentage=0.8, max_percentage=1.2)], p=0.5)
        # augment_compose = augment3D.RandomChoice(transforms=[augment3D.RandomRotation(min_angle=-5, max_angle=5)], p=0.5)
        augment_compose = augment3D.RandomChoice(transforms=[augment3D.RandomRotation(min_angle=-5, max_angle=5),
                                                             augment3D.RandomZoom(min_percentage=0.8, max_percentage=1.2)],p=0.5)
        if args.isAugment:
            images_tensor = np.zeros_like(images_tensor)
            for i in index:
                images_tensor[i] = augment_compose(images_tensor[i])
...
# Función original, solo soporta entre [0, 1]
