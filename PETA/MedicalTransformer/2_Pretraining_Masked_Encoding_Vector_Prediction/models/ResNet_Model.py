...
class Multiview_MEP(nn.Module):
    def __init__(self, args, device):
        ...
        # transformer
        # print("transformrer args")
        # print(self.args)
        self.device = device
        self.TF = Transformer(self.args, device)
    ...
    def forward(self, x):
        ...
        masked_x = Variable((-1) * torch.ones(x_axial.size(0), x_axial.size(2), x_axial.size(3)).unsqueeze(1).unsqueeze(-1), requires_grad=True).to(self.device)  # [B, 1, 193, 229]
        ...
        mask_encoding_sagittal = self.encoding_sag(masked_x)  # [B, 256, 19]
        ...
        masked_x = Variable((-1) * torch.ones(x_coronal.size(0), x_coronal.size(2), x_coronal.size(3)).unsqueeze(1).unsqueeze(-1), requires_grad=True).to(self.device)  # [B, 1, 193, 193]
        ...
        mask_encoding_coronal = self.encoding_cor(masked_x)  # [B, 256, 19]
...
class Transformer(nn.Module):
    def __init__(self, args, device):
        ...
        self.pos_embed = PositionalEncoder(self.args, self.device)
...
class PositionalEncoder(nn.Module):
    ...
    def forward(self, x, view_idx):
        ...
        pos_emb = Variable(self.pe[:, :seq_len], requires_grad=False).to(self.device)
        # print("pos embed shape", pos_emb.shape)
        pos_emb = pos_emb.expand_as(x)

        seq = torch.tensor(view_idx * (seq_len) , dtype=torch.long).to(self.device)
...
def attention(q, k, v, d_k, mask=None, dropout=None):
    ...
    if mask is not None:
        ...
        # scores = scores.masked_fill(torch.tensor(mask, dtype=torch.uint8).to(self.device), -1e9)
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # x = self.avgpool(x)  # [1, 1024, 1, 1, 193] # comentado del original
        x = nn.AvgPool3d(kernel_size=(x.size(2), x.size(3), 1), stride=1)(x)
